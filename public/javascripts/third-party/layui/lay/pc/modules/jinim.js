...
layui.define(['jquery', 'layer', 'laytpl', 'upload'], function(exports){
  ...
  var SHOW = 'layim-show', THIS = 'jinim-this';
  ...
  var listTpl = function(options){
    ...
    return ['{{# var length = 0; layui.each('+ options.item +', function(i, data){ length++; }}'
      ,'<li layim-event="chat" data-type="'+ options.type +'" data-index="{{ '+ (options.index||'i') +' }}" id="layim-'+ options.type +'{{ data.id }}"><img src="{{ data.avatar }}"><span>{{ data.username||data.actname||data.groupname||data.name||"佚名" }}</span><p>{{ data.remark||data.sign||"" }}</p></li>'
    ,'{{# }); if(length === 0){ }}'
      ,'<li class="layim-null">'+ (nodata[options.type] || "暂无数据") +'</li>'
    ,'{{# } }}'].join('');
  };

  // 左菜单模板 wujq  2016-8-17
  var leftTpl = ['<div class="layui-layim">'
    ,'<div class="layim-tab-content  {{# if(d.base.isfriend){ }}layim-show{{# } }} layim-list-friend">'
      ,'<ul class="layui-layim-subtab">'
        ,'<li class="layui-icon layim-this" layim-event="subtab">聊天</li>'
        ,'<li class="layui-icon" layim-event="subtab">联系人</li>'
      ,'</ul>'
      /****   即时会话聊天  *****/
      ,'<div class="layim-subtab-content layim-show">'
      ...
      ,'</div>'
      /**** 联系人列表 ****/
      ,'<div class="layim-subtab-content">'
        ,'{{# layui.each(d.friend, function(index, item){ }}'
            ,'<ul class="layui-layim-list layim-show">'
            ,listTpl({
              type: "friend"
              ,item: "item.list"
              ,index: "index"
    ...
    ,'{{# if(d.base.isgroupSend){ }}'
      ,'<ul class="layim-tab-content  {{# if(!d.base.isfriend && d.base.isgroupSend){ }}layim-show{{# } }}">'
        ,'<li>'
          ,'<ul class="layui-layim-list layim-show layim-list-groupSend">'
          ,listTpl({
            type: 'groupSend'
          })
          ,'</ul>'
  ...
  // 聊天内容模板
  var elemChatMain = ['{{# if(d.type === "svmessage"){ }}' // 添加系统消息处理类型  wjq 2016-8-8
      ,'<li><div style="height: 100px;background: #f3f3f3;width: 450px;padding:10px;">'
      ,'<h2 style="border-bottom:1px solid #ccc;padding-bottom:5px;">{{ d.title||"佚名" }}  ---  {{ layui.laytpl.date(d.timestamp) }}</h2>'
      ,'<h3>{{ d.content||"佚名" }}</h3></div></li>'
    ,'{{# } else { }}'
  ...
  var cache = {message: {}, chat: []}, init = function(options){
    ...
    post(options.init, function(data){
      ...
      $("#jinim-chat-list").html(laytpl(leftTpl).render(obj));

      //按最新会话重新排列
      var arr = [], historyElem = layimMain.find('.layim-list-history');
  ...
  //显示主面板
  var layimMain = $("#chat-main"), popim = function(content){
    return layer.open({
      type: 1
      ,area: ['300px', '520px']
  ...
  //显示聊天面板
  var layimChat = $("#chat-content"), layimMin, chatIndex, To = {}, popchat = function(data){
    data = data || {};
    ...
    if(chat[0]){
      ...
      $("#chat-content").html(laytpl(elemChatTpl).render(render));
  ...
  var setChatMin = function(newMsg){
    ...
    layer.open({
      ...
      ,content: '<span class="svmuu-color-red">'+ thatChat.name +'</span>'
  ...
  var getMessage = function(data){
    ...
    var group = {},svmessage = {}, index = elem.index();
    ...
    if((!layimChat && data.content) || index === -1){
      if(cache.message[data.type + data.id]){
        ...
        if(data.type === 'friend'){
          ...
          /**-----------svmessage--end------------**/ 
        } else if(data.type === 'groupSend'){
          /**----------系统消息处理  svmessage-  wjq  2016-8-8-------start-----**/
          var isgroupSend;
          layui.each(cache.groupSend, function(index, item){
            if(item.id == data.id){
              item.type = 'groupSend';
              item.name = item.username;
              cache.chat.push(item);
          ...
          /**-----------svmessage--end------------**/ 
        } else if(data.type === 'autoReplay'){
          /**----------系统消息处理  svmessage-  wjq  2016-8-8-------start-----**/
          var isautoReplay;
          layui.each(cache.autoReplay, function(index, item){
            if(item.id == data.id){
              item.type = 'autoReplay';
              item.name = item.username;
              cache.chat.push(item);
              return ismessage = true;
            }
  ...
  var anim = 'layui-anim-up', events = {
    ...
    // 添加tab切换时间
    ,subtab: function(othis){
      var index, main = '.layim-subtab-content';
      var tabs = layimMain.find('.layui-layim-subtab>li');
      typeof othis === 'number' ? (
        index = othis
        ,othis = tabs.eq(index)
      ) : (
